...
describe("ConfirmDialog", () => {
  ...
  describe("requireReason (LIMS compliance)", () => {
    it("blocks confirmation until a reason is typed", async () => {
      ...
      await waitFor(() =>
        expect(screen.getByText("limsChangeReasonRequired")).toBeInTheDocument()
      );
    ...
    it("passes the trimmed reason through to onConfirm", async () => {
      ...
      fireEvent.change(reasonBox(), {
        target: { value: "  Relocated to Block C  " }
      });
      fireEvent.click(confirmButton());

      await waitFor(() =>
        expect(onConfirm).toHaveBeenCalledWith("Relocated to Block C")
      );
    });
    ...
    it("clears the validation error once the user starts typing", async () => {
      ...
      await waitFor(() =>
        expect(
          screen.queryByText("limsChangeReasonRequired")
