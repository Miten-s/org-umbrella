...
const describeChildDelta = (delta: {
  added?: unknown[];
  removed?: unknown[];
  changed?: unknown[];
}): string => {
...
const diffEntry = (entry: LimsAuditEntry): DiffRow[] => {
    ...
    return [
      {
        key: entry.id,
        field: entry.field ?? "",
        oldValue: readableValue(entry.oldValue),
        newValue: readableValue(entry.newValue)
      }
    ];
  }
  ...
  for (const key of keys) {
    ...
    if (isDataUri(oldObj?.[key]) || isDataUri(newObj?.[key])) {
      const before = isDataUri(oldObj?.[key])
        ? ""
        : readableValue(oldObj?.[key]);
      const after = isDataUri(newObj?.[key])
        ? humanizeUpdateLabel(key)
        : readableValue(newObj?.[key]);
  ...
  return rows.length
    ? rows
    : [{ key: entry.id, field: "", oldValue: "", newValue: "" }];
};
...
const ClampedText = ({ value }: { value: string }) =>
  value.length > 160 || value.includes("\n") ? (
    <div className="max-h-24 max-w-xs overflow-y-auto whitespace-pre-wrap break-words">
      {value}
    </div>
  ) : (
                  ...
                  {entries.flatMap((entry) =>
                    diffEntry(entry).map((row, index) => (
                      <tr key={row.key}>
                        ...
                        <td className="max-w-xs px-4 py-3 text-sm text-gray-500 dark:text-gray-400">
                          {index === 0 ? (
                            <ClampedText value={entry.changeReason ?? ""} />
                          ) : (
                            ""
                          )}
                        </td>
