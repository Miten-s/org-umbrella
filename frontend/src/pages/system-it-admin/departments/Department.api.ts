...
export const fetchDepartmentList = async (
  ...
  return toListResult<Department>(
    response.data,
    params,
    DATA_KEYS,
    RELATION_KEYS
...
export const fetchDepartmentOptions = async (
  ...
  return toOptionsPage<Department>(
    response.data,
    params,
...
export const fetchDepartmentById = async (id: string, signal?: AbortSignal) => {
  ...
  return normalizeIdWithRelations(
    response.data?.department ?? response.data,
    RELATION_KEYS
  ) as Department;
};
...
export const bulkUpdateDepartment = async (
  updates: { id: string; payload: DepartmentPayload }[]
) => {
