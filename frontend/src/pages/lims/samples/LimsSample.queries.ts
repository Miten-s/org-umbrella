...
import {
  ...
  bulkRestoreLimsSample,
  bulkUpdateLimsSample,
  createLimsSample,
...
export const useCreateLimsSample = () => {
  ...
  return useMutation({
    mutationFn: ({
      ...
      files?: File[];
    }) => createLimsSample(payload, files),
    onSuccess: () => {
...
export const useBulkRestoreLimsSample = () => {
  const invalidate = useInvalidate();
  return useMutation({
    mutationFn: ({
      selection,
      ...
      selection: BulkSelection;
      changeReason: string;
    }) => bulkRestoreLimsSample(selection, changeReason),
    onSuccess: (_data, { selection }) => {
      const count = selection.mode === "ids" ? selection.ids.length : undefined;
      toast(
        count && count > 1
...
export const useBulkCopyLimsSample = () => {
  const invalidate = useInvalidate();
  return useMutation({
...
export const useBulkUpdateLimsSample = () => {
  ...
  return useMutation({
    mutationFn: ({
      ...
      changeReason: string;
    }) => bulkUpdateLimsSample(updates, changeReason),
    onSuccess: (data) => {
      toast(
        data.count > 1
