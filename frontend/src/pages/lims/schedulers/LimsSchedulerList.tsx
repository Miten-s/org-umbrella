...
import LimsSchedulerForm, { type LimsSchedulerFormMode } from "./LimsSchedulerForm";
...
const LimsSchedulerList = () => {
  ...
  const [active, setActive] = useState<LimsScheduler | null>(null);
  ...
  const fetchList = useCallback(
    (params: Parameters<typeof fetchLimsSchedulerList>[1], signal?: AbortSignal) =>
  ...
  const label = (row: LimsScheduler) => String(row.schedulerId ?? row.name ?? "");
