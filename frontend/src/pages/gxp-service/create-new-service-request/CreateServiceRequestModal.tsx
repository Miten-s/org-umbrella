...
const CreateServiceRequestModal = ({
  ...
  const [moduleBaseOptions, setModuleBaseOptions] = useState<
    MultiSelectOption[]
  >([]);
  ...
  const onFormSubmit: SubmitHandler<ServiceRequestFormInput> = async (data) => {
    const parsed: ServiceRequestFormOutput =
      getServiceRequestSchema.parse(data);
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        const baseEnvironmentOptions = extractList<any>(envs, [
          "environments",
          "data"
        ]).map((env) => toDropdown(env, "environmentName"));
        const baseAssignmentGroupOptions = extractList<any>(groups, [
          "assignmentGroups",
          "data"
        ]).map((group) => toDropdown(group, "groupName"));
        const baseWorkflowOptions = extractList<any>(wfs, [
          "workflows",
          "data"
        ]).map((wf) => toDropdown(wf, "workflowName"));

        const initialEnvironment =
          (resolvedInitial as any)?.environment ??
          (resolvedInitial as any)?.applicationEnvironment;
        const initialAssignmentGroup =
          (resolvedInitial as any)?.assignmentGroup ??
          (resolvedInitial as any)?.group;
        const initialWorkflow =
          (resolvedInitial as any)?.workflow ??
        ...
        const initialAssignmentGroupId = normalizeMixedId(
        ...
        setEnvironmentOptions(
          mergeUniqueDropdownOptions(
            ...
            extraEnvironmentOptions
          )
        );
        setAssignmentGroupOptions(
          mergeUniqueDropdownOptions(
            baseAssignmentGroupOptions,
            extraAssignmentGroupOptions
          )
        );
        setWorkflowOptions(
          mergeUniqueDropdownOptions(baseWorkflowOptions, extraWorkflowOptions)
        );
        const moduleOptionsList = extractList<any>(modules, [
          "modules",
          "software",
          "data"
        ]).map((mod) => ({
        ...
        setModuleBaseOptions(
          moduleOptionsList.filter((opt) => opt.value && opt.text)
        );
        setRoleBaseOptions(
  ...
  }, [appDetails, moduleBaseOptions, resolvedInitial, roleBaseOptions]);
