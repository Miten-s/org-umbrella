...
export const fetchAssignmentGroupById = async (
  id: string,
  signal?: AbortSignal
) => {
...
export const fetchAssignmentGroupOptions = async (
  ...
  return toOptionsPage<AssignmentGroup>(
    response.data,
    params,
    (row) => row.groupName,
    DATA_KEYS
...
export const createAssignmentGroup = async (
...
export const bulkCopyAssignmentGroup = async (
  records: AssignmentGroupPayload[]
) => {
  const response = await gxpApi.post(`${ROUTE}/bulk-copy`, { records });
  return response.data as AssignmentGroup[];
};

export const bulkUpdateAssignmentGroup = async (
  updates: { id: string; payload: AssignmentGroupPayload }[]
) => {
