...
export const useAssignmentGroupOptions = (args: {
  search: string;
  enabled?: boolean;
...
const useInvalidate = () => {
  ...
  return () =>
    queryClient.invalidateQueries({ queryKey: assignmentGroupKeys.all });
};

export const useCreateAssignmentGroup = () => {
  const invalidate = useInvalidate();
  return useMutation({
    mutationFn: (payload: AssignmentGroupPayload) =>
      createAssignmentGroup(payload),
    onSuccess: () => {
...
export const useBulkDeleteAssignmentGroup = () => {
  ...
  return useMutation({
    mutationFn: (selection: BulkSelection) =>
      bulkDeleteAssignmentGroup(selection),
    onSuccess: (_data, selection) => {
      const count = selection.mode === "ids" ? selection.ids.length : undefined;
      toast(
        count && count > 1
          ? `${count} assignment groups deleted successfully.`
...
export const useBulkCloneAssignmentGroup = () => {
  ...
  return useMutation({
    mutationFn: (selection: BulkSelection) =>
      bulkCloneAssignmentGroup(selection),
    onSuccess: (_data, selection) => {
      const count = selection.mode === "ids" ? selection.ids.length : undefined;
      toast(
        count && count > 1
          ? `${count} assignment groups copied successfully.`
...
export const useBulkCopyAssignmentGroup = () => {
  const invalidate = useInvalidate();
  return useMutation({
    mutationFn: (records: AssignmentGroupPayload[]) =>
      bulkCopyAssignmentGroup(records),
    onSuccess: (data) => {
      toast(
        data.length > 1
          ? `${data.length} assignment groups copied successfully.`
          : "Assignment group copied successfully.",
        "success"
      );
      invalidate();
...
export const useBulkUpdateAssignmentGroup = () => {
  const invalidate = useInvalidate();
  return useMutation({
    mutationFn: (updates: { id: string; payload: AssignmentGroupPayload }[]) =>
      bulkUpdateAssignmentGroup(updates),
    onSuccess: (data) => {
      toast(
...
export const useBulkRestoreAssignmentGroup = () => {
  ...
  return useMutation({
    mutationFn: (selection: BulkSelection) =>
      bulkRestoreAssignmentGroup(selection),
    onSuccess: (data) => {
      toast(
        data.count > 1
          ? `${data.count} assignment groups restored successfully.`
          : "Assignment group restored successfully.",
        "success"
      );
      invalidate();
    }
...
export const useToggleAssignmentGroupStatus = () => {
  ...
  return useMutation({
    mutationFn: (group: AssignmentGroup) =>
      group.isActive
        ? disableAssignmentGroup(group.groupName)
        : enableAssignmentGroup(group.groupName),
    onMutate: async (group) => {
      await queryClient.cancelQueries({ queryKey: assignmentGroupKeys.all });
      const nextActive = !group.isActive;
      const snapshots = queryClient.getQueriesData<ListResult<AssignmentGroup>>(
        { queryKey: assignmentGroupKeys.all }
      );
      snapshots.forEach(([key, data]) => {
        ...
        queryClient.setQueryData<ListResult<AssignmentGroup>>(key, {
          ...
          rows: data.rows.map((row) =>
            row.id === group.id ? { ...row, isActive: nextActive } : row
          )
        });
      });
      return { snapshots };
    ...
    onSuccess: (_data, group) =>
      toast(
        group.isActive
          ? "Assignment group disabled successfully."
          : "Assignment group enabled successfully.",
        "success"
      ),
