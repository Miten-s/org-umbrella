...
export const useWorkflowOptions = (args: {
  search: string;
  enabled?: boolean;
...
export const useBulkDeleteWorkflow = () => {
  ...
  return useMutation({
    ...
    onSuccess: (_data, selection) => {
      ...
      toast(
        count && count > 1
          ? `${count} workflows deleted successfully.`
...
export const useBulkCloneWorkflow = () => {
  ...
  return useMutation({
    ...
    onSuccess: (_data, selection) => {
      ...
      toast(
        count && count > 1
          ? `${count} workflows copied successfully.`
...
export const useBulkCopyWorkflow = () => {
  ...
  return useMutation({
    ...
    onSuccess: (data) => {
      toast(
        data.length > 1
          ? `${data.length} workflows copied successfully.`
          : "Workflow copied successfully.",
        "success"
      );
      invalidate();
...
export const useBulkUpdateWorkflow = () => {
  const invalidate = useInvalidateWorkflows();
  return useMutation({
    mutationFn: (updates: { id: string; payload: WorkflowPayload }[]) =>
      bulkUpdateWorkflow(updates),
    onSuccess: (data) => {
      toast(
        data.results.length > 1
          ? `${data.results.length} workflows updated successfully.`
...
export const useBulkRestoreWorkflow = () => {
  ...
  return useMutation({
    ...
    onSuccess: (data) => {
      toast(
        data.count > 1
          ? `${data.count} workflows restored successfully.`
...
export const useToggleWorkflowStatus = () => {
  ...
  return useMutation({
    ...
    onMutate: async (workflow) => {
      ...
      const snapshots = queryClient.getQueriesData<ListResult<Workflow>>({
        queryKey: workflowKeys.all
      });
      snapshots.forEach(([key, data]) => {
        ...
        queryClient.setQueryData<ListResult<Workflow>>(key, {
          ...
          rows: data.rows.map((row) =>
            row.id === workflow.id ? { ...row, status: nextStatus } : row
          )
        });
      });
      return { snapshots };
