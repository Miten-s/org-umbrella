  ...
  const bulkActions = useMemo<DataTableBulkAction[]>(
    () => [
      ...
      },
      {
        ...
        permission: GXP_PERMISSIONS.UPDATE_USER,
        hidden: (rows) =>
          !(rows as GxpUser[]).some((row) => row.status === "disabled"),
        onClick: (selection) => {
          if (selection.mode !== "ids") {
  ...
  const rowActions = useMemo<AppDataTableRowAction<GxpUser>[]>(
    () => [
      ...
      },
      {
        ...
        onClick: (u) => {
          setPendingRestore({ mode: "ids", ids: [u.id] });
          setRestoreNames([u.user.name || "-"]);
        }
      },
      {
        key: "delete",
      ...
      <ConfirmDialog
        ...
        description={
          restoreNames.length > 1
            ? `Are you sure you want to restore these ${restoreNames.length} users?`
