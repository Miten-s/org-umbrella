...
export const useEnvironmentOptions = (args: {
  search: string;
  enabled?: boolean;
...
export const useBulkDeleteEnvironment = () => {
  ...
  return useMutation({
    ...
    onSuccess: (_data, selection) => {
      ...
      toast(
        count && count > 1
          ? `${count} environments deleted successfully.`
...
export const useBulkCopyEnvironment = () => {
  ...
  return useMutation({
    ...
    onSuccess: (data) => {
      toast(
        data.length > 1
          ? `${data.length} environments copied successfully.`
...
export const useBulkUpdateEnvironment = () => {
  const invalidate = useInvalidateEnvironments();
  return useMutation({
    mutationFn: (updates: { id: string; payload: EnvironmentPayload }[]) =>
      bulkUpdateEnvironment(updates),
    onSuccess: (data) => {
      toast(
        data.results.length > 1
          ? `${data.results.length} environments updated successfully.`
...
export const useBulkRestoreEnvironment = () => {
  ...
  return useMutation({
    ...
    onSuccess: (data) => {
      toast(
        data.count > 1
          ? `${data.count} environments restored successfully.`
...
export const useToggleEnvironmentStatus = () => {
  ...
  return useMutation({
    ...
    onMutate: async (environment) => {
      ...
      const nextStatus =
        environment.status === "enabled" ? "disabled" : "enabled";
      const snapshots = queryClient.getQueriesData<ListResult<Environment>>({
        queryKey: environmentKeys.all
      });
      snapshots.forEach(([key, data]) => {
        ...
        queryClient.setQueryData<ListResult<Environment>>(key, {
          ...
          rows: data.rows.map((row) =>
            row.id === environment.id ? { ...row, status: nextStatus } : row
          )
        });
      });
      return { snapshots };
    ...
    onSuccess: (_data, environment) =>
      toast(
        environment.status === "enabled"
          ? "Environment disabled successfully."
          : "Environment enabled successfully.",
        "success"
      ),
...
export const useBulkCloneEnvironment = () => {
  ...
  return useMutation({
    ...
    onSuccess: (_data, selection) => {
      ...
      toast(
        count && count > 1
          ? `${count} environments copied successfully.`
          : "Environment copied successfully.",
        "success"
      );
      invalidate();
