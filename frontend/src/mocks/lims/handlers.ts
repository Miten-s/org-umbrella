  ...
  return [
    ...
    http.post(
      ...
      async ({ request }: { request: Request }) => {
        ...
        const count = softDelete(
          entity,
          ids,
          body.changeReason as string | undefined
        );
    ...
    http.get(url(route), ({ request }: { request: Request }) => {
      const { rows, metadata } = queryList(entity, readListArgs(request));
      return HttpResponse.json({ [entity.dataKey]: rows, metadata });
    }),

    http.post(url(route), async ({ request }: { request: Request }) =>
      HttpResponse.json(createRow(entity, await readBody(request)), {
        status: 201
      })
    ),
...
const phraseEntriesHandler = http.get(
  url("lims-phrases/entries"),
...
export const limsHandlers = [
  phraseEntriesHandler,
  permissionsHandler,
  ...entityHandlers
];
