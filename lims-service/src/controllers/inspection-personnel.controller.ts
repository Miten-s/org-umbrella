...
export const bulkDelete = asyncHandler(async (req: Request, res: Response) => {
  ...
  const count = await inspectionPersonnelService.bulkDelete(ids, changeReason, userId, userName);
...
export const bulkDuplicate = asyncHandler(async (req: Request, res: Response) => {
  ...
  const userId = String(req.user?.id || "system");
  const userName = String(req.user?.fullName || "system");
  const count = await inspectionPersonnelService.bulkDuplicate(ids, userId, userName);
...
export const restore = asyncHandler(async (req: Request, res: Response) => {
  ...
  const userId = String(req.user?.id || "system");
  const userName = String(req.user?.fullName || "system");
  const restored = await inspectionPersonnelService.restore(id, changeReason, userId, userName);
...
export const getAuditLogs = asyncHandler(async (req: Request, res: Response) => {
  ...
  const result = await inspectionPersonnelService.getAuditLogs(id, page, limit);
