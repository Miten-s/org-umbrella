...
export const attachCancelRoutes = <M extends Model>(
  ...
  const transition = (
    ...
    asyncHandler(async (req: Request, res: Response) => {
      ...
      const groupId = (record as Model & { groupId?: string | null }).get?.(
        "groupId"
      ...
      if (
        ...
        return res
          .status(403)
          .json({ message: `${entityName} is outside your groups.` });
      }
  ...
  router.patch(
    "/:id/cancel",
    authorize(permissionEntity, "UPDATE"),
    transition("Cancelled", "CANCEL")
  );
  router.patch(
    "/:id/reactivate",
    authorize(permissionEntity, "UPDATE"),
    transition("Open", "REACTIVATE")
  );
